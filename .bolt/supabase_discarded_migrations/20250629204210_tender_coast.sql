/*
  # Complete ReelApps Database Schema with Sample Data

  1. Core Tables
    - `profiles` - User profiles (candidates and recruiters)
    - `job_postings` - Job listings created by recruiters
    - `job_applications` - Applications submitted by candidates
    - `candidate_skills` - Skills associated with candidates
    - `candidate_projects` - Projects showcased by candidates
    - `candidate_experiences` - Work experience records
    - `persona_analyses` - AI personality analysis results
    - `reviews` - Peer reviews and recommendations
    - `ai_match_results` - AI-generated candidate matches
    - `recruitment_analytics` - Analytics and metrics

  2. Security
    - Enable RLS on all tables
    - Add appropriate policies for data access
    - Ensure recruiters can only see their own data

  3. Sample Data
    - Sample candidates with skills and projects
    - Sample recruiters and job postings
    - AI analysis results for testing
*/
...
CREATE POLICY "Recruiters can read candidate profiles" ON profiles
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Candidates can read active job postings" ON job_postings
  FOR SELECT USING (
    status = 'active' 
    AND EXISTS (
...
CREATE POLICY "Candidates can manage own applications" ON job_applications
  FOR ALL USING (candidate_id = auth.uid());

CREATE POLICY "Recruiters can see applications to their jobs" ON job_applications
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Recruiters can update applications to their jobs" ON job_applications
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM job_postings 
      WHERE id = job_applications.job_id 
...
CREATE POLICY "Users can manage own skills" ON candidate_skills
  FOR ALL USING (candidate_id = auth.uid());

CREATE POLICY "Recruiters can read candidate skills" ON candidate_skills
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = auth.uid() 
...
CREATE POLICY "Users can manage own projects" ON candidate_projects
  FOR ALL USING (candidate_id = auth.uid());

CREATE POLICY "Recruiters can read candidate projects" ON candidate_projects
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = auth.uid() 
...
CREATE POLICY "Users can manage own experiences" ON candidate_experiences
  FOR ALL USING (candidate_id = auth.uid());

CREATE POLICY "Recruiters can read candidate experiences" ON candidate_experiences
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = auth.uid() 
...
CREATE POLICY "Users can manage own persona analysis" ON persona_analyses
  FOR ALL USING (candidate_id = auth.uid());

CREATE POLICY "Recruiters can read persona analyses" ON persona_analyses
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = auth.uid() 
...
CREATE POLICY "Recruiters can read reviews" ON reviews
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE id = auth.uid() 
...
CREATE POLICY "Recruiters can see match results for their jobs" ON ai_match_results
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM job_postings 
...
CREATE POLICY "Candidates can see their own match results" ON ai_match_results
  FOR SELECT USING (candidate_id = auth.uid());

CREATE POLICY "Recruiters can manage own analytics" ON recruitment_analytics
  FOR ALL USING (recruiter_id = auth.uid());
